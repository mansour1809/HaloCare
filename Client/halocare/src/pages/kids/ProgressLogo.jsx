// components/kids/ProgressLogo.jsx - גרסה מעודכנת
...
const FormsGrid = styled(Grid)(({ theme }) => ({
...
const FormChip = styled(Chip)(({ status, theme }) => {
  const getColors = () => {
    switch (status) {
      case 'completed':
...
const ProgressLogo = ({ 
  ...
  return (
    <IntegratedContainer>
      ...
      >
        <FormsGrid container spacing={1} justifyContent="center">
          {forms
            .sort((a, b) => a.formOrder - b.formOrder)
            .map((form, index) => (
              <Grid item key={form.formId}>
                <FormChip
                  status={form.status}
                  icon={getFormIcon(form.status)}
                  label={form.formName}
